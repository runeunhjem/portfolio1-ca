...
function loadCurrentImage(container, position) {
  const projectClass = container.parentNode.parentNode.classList[1];
...
function handleArrowClick(event) {
  ...
  const maxPosition = imageUrls[carouselContainer.dataset.projectClass].length - 1;

  let newPosition;

  if (arrow.classList.contains("left-arrow")) {
    newPosition =
      (currentPosition - 1 + imageUrls[carouselContainer.dataset.projectClass].length) %
      imageUrls[carouselContainer.dataset.projectClass].length;
  } else if (arrow.classList.contains("right-arrow")) {
    newPosition = (currentPosition + 1) % imageUrls[carouselContainer.dataset.projectClass].length;
  }

  carouselContainer.dataset.position = newPosition;
  loadCurrentImage(carouselContainer, newPosition);
}

carouselContainers.forEach((container) => {
  const projectClass = container.parentNode.parentNode.classList[1];
  container.dataset.projectClass = projectClass;
  container.dataset.position = 0;
  loadCurrentImage(container, 0);
});
