...
function loadCurrentImage(container, slideDirection) {
  ...
  projectContainer.querySelector(".image").style.transform = `translateX(${slideDirection === "left" ? 100 : -100}%)`;

  // Apply slide-out animation to the previous image
  container.querySelector(".image").style.transform = `translateX(${slideDirection === "left" ? -100 : 100}%)`;

  // Update the background image of the existing image
  container.querySelector(".image").style.backgroundImage = `url(${imagePath})`;
}
