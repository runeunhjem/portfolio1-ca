...
function loadCurrentImage(container) {
  ...
  // Call updateImageCounter after loading the current image
  updateImageCounter(position, imageUrls[projectClass].length - 1, projectClass);

}
function updateImageCounter(currentPosition, maxPosition, projectClass) {
  console.log("proj");
  maxPosition = imageUrls[projectClass].length;
...
function handleArrowClick(event) {
  ...
  // Update position after loading the current image
  carouselContainer.dataset.position = newPosition;

  // Call updateImageCounter after updating the position
  updateImageCounter(newPosition, imageUrls[projectClass].length - 1, projectClass);
}
